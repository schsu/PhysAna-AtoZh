...
while(my $current_line=<INFILE1>){
	...
	unless($current_line=~/^\s/ || $current_line=~/#/ || $limit==0){	
		...
		my $led=0;
		if(@couplings_list==1){
			if(lc($ident) eq  "gal" || lc($ident) eq  "gad" || lc($ident) eq  "gau" || lc($ident) eq  "gwf" || lc($ident) eq  "gzn" || lc($ident) eq  "gzl" || lc($ident) eq  "gzd" || lc($ident) eq  "gzu" || lc($ident) eq  " gw" || lc($ident) eq  "gwwa" || lc($ident) eq  "gwwz"|| lc($ident) eq  "gwfc" || lc($ident) eq  "gwfs" || lc($ident) eq  "gwfm"|| lc($ident) eq  "gg"){$led=1;}
		}
		for(my $u=0;$u<@couplings_list-1;$u++){
			if((lc($ident) eq lc($couplings_list[$u])) || lc($ident) eq  "gal" || lc($ident) eq  "gad" || lc($ident) eq  "gau" || lc($ident) eq  "gwf" || lc($ident) eq  "gzn" || lc($ident) eq  "gzl" || lc($ident) eq  "gzd" || lc($ident) eq  "gzu" || lc($ident) eq  " gw" || lc($ident) eq  "gwwa" || lc($ident) eq  "gwwz"|| lc($ident) eq  "gwfc" || lc($ident) eq  "gwfs" || lc($ident) eq  "gwfm"|| lc($ident) eq  "gg"){$led=1;}#led=1 means that the vertex is already declared
		}
		if($led==0){
			if($Type==1){
				if($tag0!=0){$VT[0]=$VT[0].",";}
